...
# observation 的序列化方式。
#
# 线上 agent.py 是 `result_text = str(result)` —— 对 dict 而言这是 **Python repr**：
# 单引号、True/False/None，而不是 JSON 的双引号和 true/false/null。
# 训练数据必须匹配线上，否则模型推理时会遇到没见过的观测格式。
#
# ⚠️ 但 str(dict) 客观上是个糟糕的选择：Qwen 的工具调用能力是在 JSON 工具结果上训出来的，
# 喂 Python repr 等于让它解析一个陌生格式。建议后端把那一行改成
# `json.dumps(result, ensure_ascii=False)`（改完把这里切成 "json"）。
# 在他们改之前，默认保持与线上一致。
OBSERVATION_FORMAT = "repr"


def _serialize_result(content) -> str:
    if isinstance(content, str):
        return content  # get_time / get_weather 本来就返回 str，str() 是恒等
