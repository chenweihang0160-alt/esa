...
- **`tool_schemas.json` 应由后端导出**，不是手抄。当前 `schemas/tool_schemas.json` 与后端仓库 `backend/agent/tools/tool_schemas.json` **逐字节一致**（指纹 `417ea145`，22 个工具），是从仓库取的；根目录那份 16 工具的旧版**已废弃，不要用**。
