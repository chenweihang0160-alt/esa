...
def gen_arxiv(cfg, version, rng, all_names, out):
    ...
    replies = {
        ...
        "TCP 三次握手的过程是什么": "客户端发 SYN，服务端回 SYN-ACK，客户端再回 ACK，双方进入 ESTABLISHED。三次是必要的：两次握手无法让客户端确认服务端的接收能力。",
        "快速排序怎么实现": "选一个基准元素做 partition，把小于它的放左边、大于的放右边，然后对两侧递归。关键在 partition 的写法和基准的选取——固定选首元素在有序输入上会退化。",
        "帮我讲讲动态规划的基本思想": "两个前提：最优子结构（大问题的最优解由子问题的最优解组成）和无后效性（状态一旦确定，后续决策不依赖到达它的路径）。做题时先定状态再写转移。",
