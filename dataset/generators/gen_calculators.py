        ...
        for j, phrasing in enumerate(seed["phrasings"]):
            ...
            out.append(
                Sample(
                    ...
                    system=build_system_prompt(style=style_for_answer(answer)),
...
def gen_math_solver(seeds, rng, all_names, version, systems) -> list[Sample]:
    ...
    for idx, seed in enumerate(seeds):
        ...
        # 把 sympy 字符串渲染成 LaTeX 是模型自己该做的一步（MATH_PRMOPT 要求包在 $ 里），
    ...
    for idx, seed in enumerate(seeds):
        ...
        out.append(
            Sample(
                ...
                schema_version=version,
                system=build_system_prompt(style=style_for_answer(seed["answer"])),
