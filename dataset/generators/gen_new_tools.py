...
from esa.system_prompt import build_system_prompt, style_for_answer  # noqa: E402
...
def mk(sid, tpl, category, system, tools, turns, version, rng, all_names, topic=""):
    return Sample(
        ...
        system=build_system_prompt(style=style_for_answer(turns[-1].content or "")), tool_names=pick_tool_names(tools, all_names, rng),
...
def _answer_for(tool: str, result: dict, kp: str) -> str:
    ...
    if tool == "record_answer":
        return (f"记下了。「{result['kp_id']}」的掌握度从 {result['mastery_before']} "
                f"更新到 **{result['mastery_after']}**，累计练习 {result['practice_count']} 次。")
    return "好的。"
...
CONFUSION_ARGS = {
    "recommend_practice": lambda course, kp: {"course": course, "weeks_to_exam": 4},
    "get_mastery_report": lambda course, kp: {"course": course},
    "get_core_memories": lambda course, kp: {},
    "record_answer": lambda course, kp: {"kp_id": kp, "correct": True},
    "get_mastery_level": lambda course, kp: {"kp_id": kp},
}
...
def gen_confusion(cfg, wrong_tool, group, phrasings, right_tool, rng, all_names, version, out):
    ...
    for j, p in enumerate(phrasings):
        ...
        query = p.format(kp=kp, kp2="链表")
        args = CONFUSION_ARGS[right_tool](course, kp)
        try:
